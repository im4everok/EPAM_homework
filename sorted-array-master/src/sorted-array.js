...
const sortedByRating = () => {
    return products.slice().sort((x, y) => +x.ratingReviews.replace(/\D/g, '') >
    +y.ratingReviews.replace(/\D/g, '') ? -1 : 1);
};

const sortedByPrice = () => {
    function compare(a, b){
        let aPrice = (a.price.newUan ? a.price.newUan : a.price).replace(/\s+/g, '').replace(/\D/g,'');
        let bPrice = (b.price.newUan ? b.price.newUan : b.price).replace(/\s+/,'').replace(/\D/g,'');
        return parseInt(aPrice, 10) > parseInt(bPrice, 10) ? -1 : 1;
    }
    let arr = products.slice().sort(compare);
  return arr;
  };
